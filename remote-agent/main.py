...
session.json 由上位机写入，字段：
  sessionId, agentToken, signalingUrl, iceServers
"""
...
def main() -> None:
    ...
    parser.add_argument("--config", required=True, help="session json path")
    parser.add_argument("--log-dir", default="", help="optional log directory")
    args = parser.parse_args()
    cfg_path = Path(args.config)
    cfg = json.loads(cfg_path.read_text(encoding="utf-8"))
    log_dir = Path(args.log_dir) if args.log_dir else cfg_path.parent
    _setup_log(log_dir)
    try:
