...
工站 test_base / 各工站
        ...
        ▼
┌─────────────────── 管理协议层 ───────────────────┐  工站唯一入口（可选）
│  QProtocolManager / QModbusManager / QSerialAmmeterManager   │
└────────────────────────┬─────────────────────────┘
                         ▼
...
│
├── scpi/
│   ├── codec/scpi_line_codec      # SCPI 行缓冲（已从 qusb 抽出）
...
│
├── visa_instrument/                 # 【域 D】VISA 仪器 + CMW
│   ├── access/                      # VisaCmd、设备 Profile
│   ├── protocol/                    # qvisa（SCPI 行读写）
│   └── cmw/                         # qcmw、cmw_gprf_facade（GPRF 业务）
│
├── product_serial/                  # 【域 E】产品串口（**独立域，不得迁入 peripheral_protocol**）
...
| **VISA** | `agreement/qvisa` 内混会话与 SCPI | 会话打开/ `viWrite` / `viRead` 下沉 **driver/visa**；`qvisa` 只做协议字符 |
...
```text
agreement/scpi/                      # 协议：行缓冲、session
scpi/device/huiling_wfp60h_scpi/     # 会凌 WFP60H（SCPI 命令表、程控/测量）

agreement/modbus/rtu/                # 协议：RTU 粘包（PDU 用 modbus/codec）
modbus/device/<前缀>_<型号>_rtu/     # Modbus RTU 电流表（非 SCPI）

serial_ammeter/manager/              # 工站入口（与 modbus/manager 平级，待建）；COM 上按配置路由到 scpi/device 或 modbus/device
```
...
### 3.5 visa_instrument 域（VISA + CMW）

| 层 | 内容 |
|----|------|
| 驱动 | `platform/driver/visa`：资源字符串、open/close |
| 协议字符 | `qvisa`：`VisaCmd::WriteLine` / `QueryLine` |
| 访问 | `VisaDeviceProfile`、地址来自工站 UI 或 SETTINGS |
| 业务 | `cmw/qcmw`、`cmw_gprf_facade`：GPRF ARB、切频、burst；`Qcmw::visaTraceEnabled()` 读 `BlePer/CmwVisaTrace` |

自由工站并联 CMW 与产品串口 PER **并行**，不走 `QProtocolManager`。
...
2. 从 `qvisa` 抽出 visa 会话到 `driver/visa`（**未做**，可选）。
...
3. ~~`qcmw` → `visa_instrument/cmw`~~ → **已完成**。
4. **未做**：`qvisa` → `visa_instrument/protocol`；`driver/visa` 会话下沉（阶段 2 可选）。
...
7. **未做**：SCPI 串口层、`qusb` 废弃；modbus 独立 pro 冒烟。
7. 验收：静态电流、吸力、程控电源 SCPI；HQ/LX RTU 读电流（**请本地 qmake + 全量编译**）。
...
| 串口读电流 | `usb->sendPowerInstruction` 等 | `device/<brand>_<model>` + scpi/modbus_rtu |
| 按键 PLC | `modbusManager` + `PlcCmd` | modbus |
| 自由工站 PLC 步骤 | `plcFacade_.run`（注入 `modbusManager`） | modbus + business |
| 并联 CMW | `cmwFacade_.run` / `Qcmw` + `visa` | visa_instrument |
...
## 7. qusb → serial_ammeter 拆分示意（类关系）
...
flowchart TB
    ...
    subgraph scpi_dev [scpi/device]
        HL[huiling_wfp60h_scpi]
    end
    subgraph modbus_dev [modbus/device]
        PLC[inovance_h5u_tcp]
        RTU[xxx_rtu 电流表]
    end
    subgraph mgr [serial_ammeter/manager]
        M[QSerialAmmeterManager]
    end
    subgraph proto [协议目录]
        SCPI[agreement/scpi]
        RTUlayer[agreement/modbus/rtu]
    end
    subgraph modbus_codec [agreement/modbus/codec]
        PDU[qmodbus_pdu]
    end
    subgraph drv [platform/driver/serial]
        SER[SerialChannel]
    end
    TB --> M
    M --> HL
