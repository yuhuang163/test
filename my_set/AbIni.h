...
#endif
#include <qserialport.h>     // 串口通信�?
#include <qserialportinfo.h> // 串口信息�?

#include <QAuthenticator>
#include <QDateTime>      // 日期和时间操�?
#include <QDebug>         // 调试输出
#include <QDesktopWidget> // 桌面信息
#include <QDir>           // 目录操作
#include <QFile>          // 文件操作
#include <QFile>
#include <QFileDialog>  // 文件对话�?
#include <QInputDialog> // 用户输入对话�?
...
// V3Pro2604290383   这个是过程条�?
// V3P0R1913G00260328000011
// V3P0DFA110001D5D1FMB63P00007
// AIR1123456C29BA91BMB5A700501  这个是pcba的sn

// [PCBA_VER] 增加治具错误码，打印船运电流，只有q系列才会进纯享一开机的时�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
// [QC_VER] 增加y20ps，y30，y30s，按键开机时长，mes多上传软件版本，轴压感卡�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
// [PRESSURE_VER] 摆幅测试调整，所有指令加换行，修复摆幅测试没勾选发指令的问�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
// [AGE_VER] 取消移除p20p的电量检�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
// [SINGLE_VER] 轴压感卡�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
// [DEBUG_VER] 搜索设备添加下拉列表，ota压测，添加勾选的互斥，取消不互斥，一机一密多上传产品key来获取秘�?立讯增加错误码，mac——sn保存路径设置，y20po改成y20ps，做y30p的适配
//
...
// [PCBA_VER] 添加轴压感版�?
//
// [IMU_VER] 增加无mes厂进船运的功�?

// [SCREEN_VER] 增加日志
// [SINGLE_VER] 修复wifi无响应问�?
// [PRESSURE_VER] 摆幅测试完成开�?
//
...
// [SINGLE_VER] 修复充电电流表格显示异常问题，设置每次打开上位机都显示tab的第一�?
// [SCREEN_VER] 设置每次打开上位机都显示tab的第一�?
// [QC_VER] 设置每次打开上位机都显示tab的第一页，优化发指令的效率
// [PRESSURE_VER] 做y20po和y25s的压感适配，添加新的双通道算法�?
// [PCBA_VER] 设置每次打开上位机都显示tab的第一页，增加对无mes的工厂适配
// [MOTOR_VER] 设置每次打开上位机都显示tab的第一�?
// [IMU_VER] 设置每次打开上位机都显示tab的第一�?
// [FREE_VER]  设置每次打开上位机都显示tab的第一�?
// [CAMERA_VER] 设置每次打开上位机都显示tab的第一�?增加对q30的适配，以及配网反初始化，测试完成重启手柄，wifi在总设置里面配置，测试的时候会自动加载wif设备名字密码ip端口，不用重启上位机
...
// [DEBUG_VER] 设置第二页增加产品串口仪�?BrushInstrument 测包/PER 与超时项
// [DEBUG_VER] 设置第二页增�?SYSTEM/ProtocolType（qpb、qfctp）下�?
// [DEBUG_VER] 测试流程可选区按产�?治具/连接/三元�?Tab 分类展示
// [CAMERA_VER]  删除冗余代码
...
// [SCREEN_VER] 添加电脑的名字存日志，增加锁定mes的功能，本地打印时间，允许重新获取压感信息一�?

// [LIGHT_VER] 添加电脑的名字存日志，增加锁定mes的功能，本地打印时间
// [FREE_VER] 添加电脑的名字存日志，增加锁定mes的功能，本地打印时间
// [FREE_VER] 仪器流程六连复位步骤，测试项 id 顺延
// [FREE_VER] 仪器流程六连停止接收与PER，非信令�?id 顺延
// [FREE_VER] 停止接收应答改长期信号槽，处理放 qfreework_data.cpp
// [FREE_VER] dongle 断开时仍推进已开始步骤的收尾，避免卡在产品串口异步步
// [FREE_VER] dongle 未连时队列当前步为产品串口仪器亦跑状态机，避免步间卡�?
// [FREE_VER] MES 电量项拆 BATTERY_INFO �?BATTERY_PERCENT，testData 仅数�?
// [FREE_VER] 并联CMW六步播放(id111�?16)，GPRF仅首次初始化、PER步内GPRF可选关 BleBrushCmwOnStopPer
//
