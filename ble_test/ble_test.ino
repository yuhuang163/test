...
bool connectToServer() {
  ...
  connect_callback = new MyClientCallback();  // 连接状态函数//有释放
  // 设置客户端回调函数
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
  ...
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    if (advertisedDevice.getAddress().equals(BLEAddress(targetDeviceAddress))) { // mac地址可以，那么准备开始连接
    ...
    case MAC:
      if (isValidMacAddress(get_cmd, length)) {
        if (connected) {  pClient->disconnect();
          // if (myDevice != nullptr) {
          //   delete myDevice;
          //   myDevice = nullptr;  // 为了避免悬空指针，最好在 delete 后将指针设为 nullptr
          // }

          // if (connect_callback != nullptr) {
          //   delete connect_callback;
          //   connect_callback = nullptr;  // 同样，为了避免悬空指针
          // }
