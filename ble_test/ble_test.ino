...

{
 ...
 digitalWrite(RST_PIN, LOW); // 将 RST_PIN 设置为高电
  delay(100);
 digitalWrite(RST_PIN, HIGH); // 将 D2_PIN 设置为高电平
...
/* 
    ...
    {
      digitalWrite(D2_PIN, LOW); // 将 D2_PIN 设置为高电平
      pBLEScan->stop();
...
if (wifiuse==0&&log==0&&blelogs==0)
  version = "dongle固件版本1.1.0(日志全关,没有wif,没有5s延迟重连,蓝牙信号日志默认关)(无wifi正式版)";


if (wifiuse==0&&log==1&&blelogs==1)
 version = "dongle固件版本1.1.0(日志全开,没有wif,没有5s延迟重连,蓝牙信号日志默认开)(无wifi调试版)";


if (wifiuse==1&&log==1&&blelogs==1)
 version = "dongle固件版本1.1.0(日志全开,没有wif,没有5s延迟重连,蓝牙信号日志默认开)(有wifi调试版)";

if (wifiuse==1&&log==0&&blelogs==0)
 version = "dongle固件版本1.1.0(日志全开,没有wif,没有5s延迟重连,蓝牙信号日志默认开)(有wifi正式版)";
...
//AT+WIFI_DATA=74:4D:BD:95:7D:EA-5674:4D:BD:95:7A:DA-45
