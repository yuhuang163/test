...
static const Row kSettings[] = {
    ...
    TIP("comboBox_factory", "MES 工厂代码（lx 立讯精密/xwd 欣旺达/xzw 欣智旺/hq 华勤技术/wks 伟克森/ydm 亚达明/byd 比亚迪/hz 华庄等）；切换后下方联动显示对应配置项。"),
    TIP("checkBox_mesRetest", "华庄 Mes/RETEST：站前检查 checkRoute 的 retest 参数。"),
    TIP("comboBox_productName", "产品型号 MES/Product_Name；切换恢复默认差异化。"),
    TIP("lineEdit_mesUrl", "MES 服务根地址（Mes/NET）；xzw、lx、wks、华庄等工厂使用。"),
    TIP("lineEdit_station", "过站工站号（Mes/machineNo）；xzw/xwd 用于 groupTest、wipTest。"),
