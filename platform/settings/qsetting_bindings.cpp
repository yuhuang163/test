...
static const Row kSettings[] = {
    ...
    CB("checkBox_blePerEnableRxTest", "BlePer/EnableRxTest", "true"),
    CB("checkBox_blePerCmwEnableFixedInit", "BlePer/CmwEnableFixedInit", "true"),
    CB("checkBox_blePerCmwBurstPollArbScount", "BlePer/CmwBurstPollArbScount", "true"),
    CB("checkBox_blePerCmwVisaTrace", "BlePer/CmwVisaTrace", "true"),
    CB("checkBox_blePerCmwQueryCurrentArbFile", "BlePer/CmwQueryCurrentArbFile", "true"),
    ...
    LE("lineEdit_blePerCmwVisaAddress", "BlePer/CmwVisaAddress", ""),
    LE("lineEdit_blePerCmwWaveformFile", "BlePer/CmwWaveformFile", "@WAVEFORM\\BT_LE_TestPacket_P9_1M.wv"),
    LE("lineEdit_blePerCmwArbCycles", "BlePer/CmwArbCycles", "1000"),
    LE("lineEdit_blePerTxCount", "BlePer/TxCount", "1000"),
    LE("lineEdit_blePerCmwTxPowerDbm", "BlePer/CmwTxPowerDbm", "-50"),
    LE("lineEdit_blePerScenarioList", "BlePer/ScenarioList", "2402:1M,2440:1M,2480:1M"),
    LE("lineEdit_blePerCmwArbPollIntervalMs", "BlePer/CmwArbPollIntervalMs", "100"),
    LE("lineEdit_blePerCmwTriggerWaitMs", "BlePer/CmwTriggerWaitMs", "1000"),
    LE("lineEdit_blePerCmwTimeoutMs", "BlePer/CmwTimeoutMs", "5000"),
    ...
    LE("lineEdit_screenInspectMinSsim", "ScreenInspect/MinSimilarity", "0.90"),
    LE("lineEdit_screenInspectDeadDiff", "ScreenInspect/DeadPixelDiff", "35"),
    LE("lineEdit_screenInspectMaxDead", "ScreenInspect/MaxDeadPixels", "8"),
    LE("lineEdit_screenInspectMura", "ScreenInspect/MuraStdMax", "22"),
    ...
    TIP("groupBox_screenInspect", "调试页/未开步骤 Gate 时的回退。检测范围在调试页拍摄图上拖拽划定，写入 ScreenInspect/Roi。"),
    ...
    TIP("groupBox_brushInstrument",
        "BrushInstrument/* 测包卡控；FreeInstrument 并联开关；BlePer/Cmw*（VISA、波形、ARB周期、场景列表等）"
        "见本分组下方输入项。详见 docs/cmw100rx.txt。"),
    TIP("lineEdit_blePerCmwVisaAddress", "BlePer/CmwVisaAddress：CMW100 VISA 资源名，并联 GPRF 必填。"),
    TIP("lineEdit_blePerCmwWaveformFile", "BlePer/CmwWaveformFile：仪侧 ARB 波形，如 @WAVEFORM\\BT_LE_TestPacket_P9_1M.wv。"),
    TIP("lineEdit_blePerCmwArbCycles", "BlePer/CmwArbCycles：ARB 周期，默认同 TxCount=1000。"),
    TIP("lineEdit_blePerTxCount", "BlePer/TxCount：PER 分母期望发包数。"),
    TIP("lineEdit_blePerCmwTxPowerDbm", "BlePer/CmwTxPowerDbm：GPRF 发射电平 dBm。"),
    TIP("lineEdit_blePerScenarioList", "BlePer/ScenarioList：频点:PHY，逗号分隔，如 2402:1M,2440:1M,2480:1M。"),
    TIP("lineEdit_blePerCmwArbPollIntervalMs", "BlePer/CmwArbPollIntervalMs：SCOunt? 轮询间隔。"),
    TIP("lineEdit_blePerCmwTriggerWaitMs", "BlePer/CmwTriggerWaitMs：触发后保持/补足毫秒（WiFi/BLE 侧）。"),
    TIP("lineEdit_blePerCmwTimeoutMs", "BlePer/CmwTimeoutMs：VISA 单次超时。"),
    TIP("checkBox_blePerEnableRxTest", "BlePer/EnableRxTest：WiFi/BLE 工站在 RSSI 通过后是否跑 PER RX。"),
    TIP("checkBox_blePerCmwEnableFixedInit", "BlePer/CmwEnableFixedInit：是否做完整 GPRF ARB 固定初始化。"),
    TIP("checkBox_blePerCmwBurstPollArbScount",
        "BlePer/CmwBurstPollArbScount：突发内轮询 SCOunt? 达周期后再补足毫秒；false 则纯定时。"),
    TIP("checkBox_blePerCmwVisaTrace", "BlePer/CmwVisaTrace：打印每条 SCPI 收发。"),
    TIP("checkBox_blePerCmwQueryCurrentArbFile",
        "BlePer/CmwQueryCurrentArbFile：初始化/突发前查询 SOURce:GPRF:GEN:ARB:FILE?。"),
    ...
    TIP("comboBox_labelPrinterName", "整机 SN 标签打印机（Printer/WindowsName）。"),
    TIP("spinBox_labelPrinterQrSize", "二维码每个模块的像素大小（Printer/QrModulePixels）。"),
    TIP("lineEdit_labelPrinterPageWidthMm", "标签纸宽度 mm（Printer/PageWidthMm，范围 5~200）。"),
    TIP("lineEdit_labelPrinterPageHeightMm", "标签纸高度 mm（Printer/PageHeightMm，范围 5~200）。"),
    TIP("pushButton_labelPrinterTestPrint", "按当前配置打印测试二维码。"),
    ...
    TIP("lineEdit_mes_login_account", "MES 登录账号（Mes/M_USERNO）。"),
    TIP("lineEdit_mes_login_password", "MES 登录密码（Mes/M_PASSWORD）。"),
    TIP("lineEdit_mes_operator", "过站操作员工号（Mes/mUserno）。"),
    TIP("lineEdit_mes_login_station", "MES 登录工站（Mes/M_MACHINENO），checkUserDo 使用。"),
    ...
    TIP("comboBox_systemProtocolType", "设备协议 qpb / qfctp / qaiot v1、v2 / qroot（W1/M8）/ qroot2（Air1），经蓝牙透传。关闭设置后立即生效，无需重启。"),
