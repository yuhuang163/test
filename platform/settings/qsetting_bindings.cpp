...
static const Row kSettings[] = {
    ...
    LE("lineEdit_remoteLogUploadUrl", "RemoteLog/UploadUrl",
       "https://fctp.luteos.com/api/factory-tool/logs/upload"),
    LE("lineEdit_remoteLogDeviceId", "RemoteLog/DeviceId", nullptr),
    LE("lineEdit_remoteLogStation", "RemoteLog/Station", "DEFAULT"),
    ...
    TIP("groupBox_logUpload", "将 exe 目录下「所有log」打包为 zip，上传到 factory-tool 日志接口。"),
    TIP("lineEdit_remoteLogUploadUrl", "RemoteLog/UploadUrl，默认 fctp 日志上传接口。"),
    TIP("lineEdit_remoteLogDeviceId", "RemoteLog/DeviceId，默认本机计算机名。"),
    TIP("lineEdit_remoteLogStation", "RemoteLog/Station，上传表单 station 字段。"),
    TIP("pushButton_uploadLogs", "压缩 所有log 目录并上传；HTTPS 失败时自动尝试 curl -k。"),
    ...
    TIP("comboBox_factory", "MES 工厂（lx/byd/wks/hz 华庄/无mes厂等）；选 hz 显示华庄专用项。"),
    ...
    TIP("comboBox_testOrderStation", "TestOrderMeta/SelectedStation。"),
    TIP("pushButton_clearConfiguredTestOrder", "清空当前工站配置区步骤。"),
