...
void MD5_Final(MD5Context* context, unsigned char* digest) {
    ...
    MD5_Encode(digest, context->state, MD5_DIGEST_LENGTH);

    memset((char*)context, 0, sizeof(*context));
}

const uint8_t* md5_gen(const uint8_t* src, uint32_t src_len) {
    static uint8_t temp[MD5_DIGEST_LENGTH];
    MD5Context ctx = {0};
    MD5_Init(&ctx);
...
void* init_md5(void) {
    MD5Context* ctx = malloc(sizeof(MD5Context));
...
void update_md5(void* ctx, void* src, int len) {
    MD5_Update(ctx, (uint8_t*)src, len);
}
