...
// convert type here.
void new_imu_calibrate::acc_update(us_matrix_float* acc_data, CalibDatasets* acc_raw, us_matrix_float* theta) {
    for (int i = 0; i < acc_raw->size; i++) {
        us_matrix_float_set(acc_data, i, 0, THETA_ACC(0) * (acc_raw->data[i].data[0] - THETA_ACC(6)));
        us_matrix_float_set(acc_data, i, 1,
                            (0 - THETA_ACC(3)) * THETA_ACC(0) * (acc_raw->data[i].data[0] - THETA_ACC(6)) +
                                THETA_ACC(1) * (acc_raw->data[i].data[1] - THETA_ACC(7)));
        us_matrix_float_set(acc_data, i, 2,
...
void new_imu_calibrate::acccalib_jacobian(us_matrix_float* J, CalibDatasets* acc_raw, us_matrix_float* theta) {
    for (int i = 0; i < FACTORY_POSE_NUM; i++) {
        ...
        us_matrix_float_set(J, i, 5, -2 * (az * (-1) * (THETA_ACC(1)) * (acc_raw->data[i].data[1] - THETA_ACC(7))));
        us_matrix_float_set(
            J, i, 6,
            -2 * (ax * (-1) * THETA_ACC(0) + ay * THETA_ACC(3) * THETA_ACC(0) + az * THETA_ACC(4) * THETA_ACC(0)));
        us_matrix_float_set(J, i, 7, -2 * (ay * (-1) * THETA_ACC(1) + az * THETA_ACC(5) * THETA_ACC(1)));
...
int new_imu_calibrate::acccalib_sensors_task() {
    ...
    if ((calib_datasets->size >= FACTORY_POSE_NUM) && (extra_calib_datasets->size >= EXTRA_POSE_NUM)) {
        ...
        for (int x = 0; x < 9; ++x) {
            qDebug() << "task:data"
                     << "x" << calib_datasets->data[x].data[0] << calib_datasets->data[x].data[1]
                     << calib_datasets->data[x].data[2];
        }
        qDebug() << "extra_datasets:" << extra_calib_datasets->data[0].data[0] << extra_calib_datasets->data[0].data[1]
                 << extra_calib_datasets->data[0].data[2];
        ...
        for (int x = 0; x < MAX_ITERATIONS; x++) {
            ...
            acc_update(acccalib_datasets_matrix, calib_datasets, imu_calib->acc_calib);
            us_blas_sgemm(CblasTrans, CblasNoTrans, 1.0, acccalib_jacobian_matrix, acccalib_value_matrix, 0.0,
                          acccalib_gradient_matrix);

            // delta=np.linalg.inv(jacobian.T@jacobian + lambda_*np.eye(9))@gradient
            us_blas_sgemm(CblasTrans, CblasNoTrans, 1.0, acccalib_jacobian_matrix, acccalib_jacobian_matrix, 0.0, jtj);
            for (int i = 0; i < 9; i++) {
            ...
            us_blas_sgemm(CblasNoTrans, CblasNoTrans, 1.0, inv, acccalib_gradient_matrix, 0.0, acccalib_delta_matrix);
            ...
            if (loss < ACC_CALIB_ACCURACY) {
                ...
                if ((us_matrix_float_get(imu_calib->acc_calib, 0, 0) > 1.3) ||
                    ...
                    emit send_imu_cali_msg(
                        "calData.kx=" + QString::number(calData.kx) + ", calData.ky=" + QString::number(calData.ky) +
                        ", calData.kz=" + QString::number(calData.kz) + ", calData.syx=" +
                        QString::number(calData.syx) + ", calData.szx=" + QString::number(calData.szx) +
                ...
                emit send_imu_cali_msg(
                    "calData.kx=" + QString::number(calData.kx) + ", calData.ky=" + QString::number(calData.ky) +
                    ", calData.kz=" + QString::number(calData.kz) + ", calData.syx=" + QString::number(calData.syx) +
                    ", calData.szx=" + QString::number(calData.szx) + ", calData.szy=" + QString::number(calData.szy) +
                    ", calData.bx=" + QString::number(calData.bx) + ", calData.by=" + QString::number(calData.by) +
                    ", calData.bz=" + QString::number(calData.bz));
...
void new_imu_calibrate::add_csvdata(CalibData* csv_data) {
    ...
    // for (int x = 0; x < 9; ++x) {
    //     qDebug() <<"task2data"<<calib_datasets->data[x].data[0]<<
    //     calib_datasets->data[x].data[1]<<calib_datasets->data[x].data[2];
    // }
}
