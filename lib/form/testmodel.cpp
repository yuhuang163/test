...
{
    connect(this, &QStandardItemModel::itemChanged, this,
            [=](QStandardItem *item)
            {
    ...
    this->setItem(itemsCounter, 2, test->testVaule);
...
void TestItems::updateTestResult()
{
    ...
    {
    case Policy::EQUAL:
        result = (equal == (testVaule->data(Qt::DisplayRole).toString())) ? PASS : FAIL;
        break;

    case Policy::BETWEEN:
        // TODO:待定义格式
        if ((testVaule->data(Qt::DisplayRole).toInt() < upperLimit) &&
            (testVaule->data(Qt::DisplayRole).toInt() > lowerLimit))
        {
            result = PASS;
        }
        else
    ...
    case Policy::GREATER_THAN:
        data = testVaule->data(Qt::DisplayRole).toInt();
