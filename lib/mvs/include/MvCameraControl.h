 ...
 *           \li GE1104 采集卡下的相机仅支持通过 MV_GIGE_DEVICE 类型枚举，不支持使用MV_GENTL_GIGE_DEVICE。
 ...
 *          @note 该接口不支持 \ref MV_CAMERALINK_DEVICE 类型设备。
 ...
 *  @remarks \li 调用该接口可以设置SDK内部图像缓存节点个数，在调用 MV_CC_StartGrabbing() 开始抓图前调用。 \n
             \li GigE Vision、USB3 Vision设备会在调用 MV_CC_StartGrabbing() 时进行内存校验，若系统内存不足，则 MV_CC_StartGrabbing() 返回异常。 \n
             \li 其他采集卡下相机，调用 MV_CC_StartGrabbing() 时检测系统内存资源不够，SDK内部将重新计算预分配的缓存节点个数。在该情况下，SDK实际分配的节点个数以重新计算的节点个数为准。
 ...
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 *  @remarks 获取到的payload大小如果想要改大，需要按照对齐字节数的整数倍增加，否则 MV_CC_AllocAlignedBuffer() 申请的内存可能出现不按要求对齐的情况
 ...
 *  D3D和OpenGL模式适用于已安装显卡驱动且显存大于1GB的设备，其客户端预览的图像效果优于GDI模式。\n
 *  根据图像大小及格式的不同，可选的渲染模式及限制如下： \n
 * - 若图像大小大于4GB：
 *    - 支持模式：OpenGL
 *    - 支持像素格式：RGB8_Packed、BGR8_Packed和Mono8
 *    - 尺寸限制：图像宽度不能超过当前环境下 OpenGL 支持的最大纹理大小。
 * - 若图像大小小于4GB：
 *    - 支持模式：可根据实际情况选择GDI、D3D或OpenGL模式。
 *    - 尺寸限制：若渲染格式为RGB8_Packed、BGR8_Packed或Mono8，图像宽高理论上可达到int类型上限，但受限于具体渲染模式的约束：
 *       - D3D模式：图像宽度不能超过 16384（最大分辨率限制为 16384 × 163840）。
 *       - OpenGL模式：图像宽度不能超过当前环境下 OpenGL 支持的最大纹理大小。      
 *  \endif
 *  \if LINUX
 ...
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 *  @remarks 发送命令设置设备的IP方式，如DHCP、LLA等，仅支持[GigE Vision]( \ref MV_GIGE_DEVICE )和[GenTL]( \ref MV_GENTL_GIGE_DEVICE )的设备。
 ...
 *  @param  handle                      [IN]            设备句柄
 *  @param  nType                       [IN]            网络传输模式，见 MV_NET_TRANS_x
    |  宏定义  |  宏定义值 |   含义  |  
    | :---: | :---: | :---: | 
 ...
 *  @remarks 增加传输包大小可以适当降低取流时的CPU占用率。但不同的PC和不同USB扩展卡存在不同的兼容性，如果该参数设置过大可能会出现取不到图像的风险。
             \if MACOS MacOS系统中，传输包大小设置过小，可能导致开始取流接口报前置条件错误，该值和图像大小与传输通道个数有关，无法给出准确的最小值，MacOS系统中尽量不做修改。 \endif
 ...
 *  @param  pcImagePath                 [IN]            存图路径，\if WINDOWS 长度不超过260字节 \endif \if LINUX 不超过255字节 \endif
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。 
 ...
 *  @remarks \li 通过该接口可以将从设备采集到的原始图像数据转换成用户所需的像素格式并存放在指定内存中。 \n
             ...
             \li 像素格式转换能力的详情，如下表所示。第一列为输入像素格式，第一行为转换后的像素格式。“  √  ”代表可以转换为目标像素格式。
                | 输入格式\输出格式 | Mono8 | RGB24 | BGR24 | RGB24 Planar | YUV422 | YUV422 YUYV |
                | ----------------- | ----- | ----- | ----- | ---------- | ------ | ----------- |
                | Mono8             |   √   |   √   |   √   |            |   √    |      √      |
                | Mono10            |   √   |   √   |   √   |            |   √    |      √      |
                | Mono10P           |   √   |   √   |   √   |            |   √    |      √      |
 ...
 *  @return   成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks  调用 MV_CC_LiquidLens_Open() 后，该接口会获取并输出镜头支持的光焦度范围。
 ...
 *  @brief  设置光焦度
 *  @param  handle                      [IN]            设备句柄
 *  @param  nFocalPower                 [IN]            光焦度值。范围由调用 MV_CC_LiquidLens_Open() 获取的镜头支持的光焦度范围决定。
 ...
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 *  @remarks \li 调用该接口前，您需调用 MV_CC_StartGrabbing() ，且需要保证当前取流策略是 \ref MV_GRAB_STRATEGY.MV_GrabStrategy_OneByOne "MV_GrabStrategy_OneByOne" 。
             \li 当nOutputImage非0时，用户可以通过注册回调或者 MV_CC_GetImageBuffer() 主动获取图像，但在对焦前建议增大图像缓存节点个数， 且如果是回调取图，回调中不要进行耗时操作，否则SDK内部会主动丢弃图像。\n
 ...
 *  @param  pUser                       [IN]            用户自定义变量
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 *  @remarks 光焦度区间功能可控制液态镜头在指定的光焦度区间内，按照设置的次数和时间，进行等间距变焦。常应用于自动对焦、景深融合等场景。
             \li 调用接口前需要用户先把相机设置为软触发模式，否则扫描过程中相机无法触发出图。相机触发出图的时机受触发延迟时间的影响，可以通过 MV_CC_LiquidLens_SetTriggerDelayTime() 设置。\n
             \li 在回调中可以获取到光焦度变化事件和扫描结束事件，如果是光焦度变化事件，消息数据返回的是当前光焦度值。\n
             @note 请勿在回调函数中执行耗时操作，否则会阻塞串口数据的读写，影响设备通信性能。\n
 ...
 *  @param  pUser                       [IN]            用户自定义变量
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 *  @remarks 多焦度轮询功能用于多段景深融合、多段景深扫描等场景。在您设置一组光焦度和保持时间后，镜头按照设置的光焦度组和保持时间轮次运行。
             \li 调用接口前需要用户先把相机设置为软触发模式，否则扫描过程中相机无法触发出图。相机触发出图的时机受触发延迟时间的影响，可以通过 MV_CC_LiquidLens_SetTriggerDelayTime() 设置。\n
             \li 在回调中可以获取到光焦度变化事件和扫描结束事件，如果是光焦度变化事件，消息数据返回的是当前光焦度值。\n
             @note
             \li 请勿在回调函数中执行耗时操作，否则会阻塞串口数据的读写，影响设备通信性能。
             \li 该接口的运行无法被中断。在执行多焦度轮询时，请等待镜头执行完成。
 ...
 *  @brief  设置触发延时
 *  @param  handle                      [IN]            设备句柄
 *  @param  nDelayTimeMs                [IN]            触发延迟时间，单位：毫秒,不能大于1000
 *  @return 成功，返回\ref 状态码 "MV_OK"；失败，返回\ref 状态码 "状态码"。
 ...
 *  @param  handle                      [IN]            It refers to the device handle.
 *  @param  nDelayTimeMs                [IN]            It refers to the trigger delay time, unit: millisecond,cannot exceed 1000.
 *  @return Returns MV_OK for success, and returns corresponding Error Code for failure.
