...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent),
    ...
    if (!pb) {
        QMessageBox::critical(this, "协议初始化失�?, "Qpb 实例初始化失败�?);
    }
    ...
    if ((selectedType == QProtocolManager::ProtocolType::Qfctp && !qfctp) ||
        ...
        QMessageBox::information(this, "协议提示", "所选协议未就绪，已自动回退�?qpb�?);
    ...
    if (SETTINGS.value("Mes/FACTORY").toString() == "hq")
        ...
        ui->statusbar->addPermanentWidget(new QLabel("欣旺�?));
...
void MainWindow::on_snInput_returnPressed() {
    ...
    if (!snRegex.match(ui->snInput->text()).hasMatch()) {
        showlog("序列号错�?);
        showlog("实际长度�? + QString::number(ui->snInput->text().length()));
        ...
        case STATE_WATI_GET_CORRECT_WIFIRSSI: {
            ...
            if (intwifirssi < HighRssi && intwifirssi > LowRssi) // wifi信号可以
            {
                ...
                if (rssitestfailcount > RssiTestTime) // wifi信号不可�?
                {
                    ...
                    qDebug() << "wifi不合格信号强�? << WIFI_RSSI;
                    ui->log->appendPlainText("wifi不合格信号强�? + WIFI_RSSI);
                    rssitestfailcount = 0;
                    at->set(DongleCmd::BleLog, 1); // 日志�?
                    state = STATE_WATI_GET_CORRECT_BLERSSI;
                }
        ...
        case STATE_WATI_GET_CORRECT_BLERSSI: {
            ...
            if (intblerssi < BleHighRssi && intblerssi > BleLowRssi) // 蓝牙信号可以
            {
                ...
                if (rssitestfailcount > RssiTestTime) // 蓝牙信号不可�?
                {
                    ...
                    bleresult = "失败";
                    qDebug() << "蓝牙不合格信号强�? << BLE_RSSI;
                    ui->log->appendPlainText("蓝牙不合格信号强�? + BLE_RSSI);
                    state = STATE_SAVE_RESULT;
                    at->set(DongleCmd::BleLog, 0); // 日志�?
                    rssitestfailcount = 0;
                }
    ...
    while (is_motor_continue) {
        ...
        case CAMERA_TEST:
            ...
            if (protocolManager.getState(static_cast<int>(Qpb::PbStateType::CameraControl))) {
                reply =
                    QMessageBox::question(this, "摄像头测�?, "摄像头正常吗�?, QMessageBox::Yes | QMessageBox::No);
...
void MainWindow::on_otaTestPushButton_clicked() {
    ...
    while (isWifiOtaContinue) {
        ...
        appendAndSaveWifiOtaLog("启动OTA，开始计�?);
        int counter = 0;
...
void MainWindow::on_otaTestPushButton_2_clicked() {
    ...
    appendAndSaveWifiOtaLog(CommonUtils::isoDateTime() + "OTA启动，开始计�?);
...
void MainWindow::on_bleotamacInput_returnPressed() {
    ...
    if (!bleOtaPressContinuing_ && ui->is_bleota_press->checkState()) {
        ...
        bleOtaPressRound_ = 0;
        appendAndSaveWifiOtaLog(QStringLiteral("蓝牙 OTA 压测开�?));
        ui->bleOtaMsg->appendPlainText(CommonUtils::isoDateTime() +
                                       QStringLiteral(" 蓝牙 OTA 压测开�?));
    }
...
bool MainWindow::connectBleForOta(const QString& mac) {
    ...
    ui->bleOtaMsg->appendPlainText(" ");
    ui->bleOtaMsg->appendPlainText(CommonUtils::isoDateTime());
    ui->bleOtaMsg->appendPlainText("开始连接蓝�?);
...
void MainWindow::startRootBleOta() {
    ...
    if (filePath.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("警告"), QStringLiteral("请选择资源或固�?OTA 文件"));
    ...
    showlog(QStringLiteral("路特 BLE OTA 开始：%1，大�?%2，CRC32=0x%3，预估块数≥%4")
                .arg(filePath)
                .arg(imageData.size())
                .arg(imageCrc32, 8, 16, QChar('0'))
                .arg(totalBlocks));
    ui->bleOtaMsg->appendPlainText(CommonUtils::isoDateTime() +
                                   QStringLiteral(" 路特 TLV OTA 开�?") + filePath);
    ui->bleOtaMsg->appendPlainText(CommonUtils::isoDateTime() +
    ...
    if (stopBleOta) {
        showlog(QStringLiteral("路特 BLE OTA 已停�?));
        ui->bleOtaMsg->appendPlainText(CommonUtils::isoDateTime() +
                                       QStringLiteral(" OTA 已停�?));
        return;
    }
...
void MainWindow::tryScheduleBleOtaPressTest(bool lastOk) {
    ...
    const QString summary =
        QStringLiteral("蓝牙 OTA 压测 �?%1 �?%2，累计成�?%3 失败 %4，本轮耗时 %5 s")
...
void MainWindow::startUsmileBleOtaTransferLegacy() {
    ...
    if (!ok || interval <= 0) {
        QMessageBox::warning(this, "警告", "无效的时间间�?);
        return;
    }
    // 断开之前的定时器连接
...
void MainWindow::on_brush_relocation_clicked() {
    protocolManager.set(DeviceCmd::MotorDampingState, 0);
    QMessageBox::warning(NULL, "警告", " 请把所有电机置�?位\t\r\n");
    protocolManager.set(DeviceCmd::MotorCali, 2);
}
...
void MainWindow::on_is_audio_mode_stateChanged(int arg1) {
    if (arg1) { //音频�?
        ...
        ui->active_picture->setText("批量处理音频文件�?);
        ui->label_95->setText("音频播放倍�?);
        ui->label_98->setText("�?);
        ui->send_audio->show();
...
void MainWindow::on_set_mode_returnPressed() {
    showlog("当前发送的模式�? + ui->set_mode->text());
...
void MainWindow::on_set_trim_data_clicked() {
    ...
    if (trimEdit == nullptr) {
        showlog("未找到trim_data_line输入�?);
        return;
    }
    bool ok = false;
    const uint32_t trimValue = trimEdit->text().toUInt(&ok, 0);
    if (!ok) {
        showlog("trim_data_line内容非法，请输入数字（支�?x前缀�?);
        return;
    }
    QVariantMap m;
    m["value"] = trimValue;
...
void MainWindow::on_send_custom_msg_clicked() {
    ...
    if (!okService || !okTlv || serviceId > 0xFFFFu || tlvType > 0xFFFFu) {
        showlog("自定义消息参数错误：serviceId/tlvType 需�?0~0xFFFF（支�?x前缀�?);
        return;
    }
