...
void TestFunctionExecutor::createTestFunctions() {
    testFunctions = {
        ...
        {"获取设备信息", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::DeviceInfo); }); }},
        {"获取基本信息", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::BaseInfo); }); }},
        {"获取外围设备状态", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::PeriphState); }); }},
        {"获取电量信息", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::Battery); }); }},
        {"进入船运模式", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::ShipMode, 1); }); }},
        {"设置UART接收状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::UartReceive, 1); }); }},
        {"设置RGB颜色",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::RgbColor, QVariant::fromValue(FacLedControl{/* params */})); }); }},
        {"设置电机校准状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorCali, 1); }); }},
        {"设置电机阻尼状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorDampingState, 1); }); }},
        {"设置电机测试状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorTestState, 1); }); }},
        {"设置工厂结果状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::FacResult, 1); }); }},

        {"设置LED颜色", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::LedColor, QVariantList{1, 1}); }); }},
        {"设置声波电机参数", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorParam, QVariantList{270, 60}); }); }},
        {"打开声波电机", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorState, 1); }); }},
        {"设置电机校准结果参数",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::MotorCaliResultParam, 100); }); }},
        {"连接WiFi",
         [&]() {
        ...
        {"设置音乐",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::Music, QByteArray("/data/audio/scan_f.bin")); }); }},
        {"设置老化测试模式",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::BurningMode, QVariantList{1, static_cast<int>(FacSwitch_START)}); }); }},
        {"设置刷牙记录",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::BrushRecord, QVariant::fromValue(FacSetBrushRecord{/* params */})); }); }},
        {"设置刷牙时间", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::BrushTime, 1625140800); }); }},
        {"设置休眠状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::Sleep, static_cast<int>(FacSwitch_START)); }); }},
        {"设置摄像头状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::CameraState, 1); }); }},
        {"设置屏幕摄像头状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::ScreenCameraState, 1); }); }},
        {"设置摄像头灯光状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::CameraLightState, 1); }); }},
        {"设置摄像头支持状态", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::CameraSupportState, 1); }); }},
        {"设置摄像头曝光时间", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::CameraExposureTime, 1000); }); }},
        {"设备复位", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::DevReset); }); }},
        {"刷牙复位", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::BrushReset); }); }},
        {"设置压力校准结果",
         [&]() {
        ...
        {"发送IMU校准结果",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::ImuCaliResult, QVariant::fromValue(ImuCalData{/* params */})); }); }},
        {"发送新的IMU校准结果",
        ...
        {"配置网络应用",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::ConfigNetworkApp, QVariant::fromValue(WifiInfo{/* params */})); }); }},
        {"断开WiFi", [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::WifiDisconnect); }); }},
        {"设置新的WiFi连接",
         [&]() {
            sendCommandWithRetry([&]() {
        ...
        {"设置压力采集参数",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::PressCollect, static_cast<int>(FacSwitch_START)); }); }},
        {"设置IMU采集参数",
         [&]() { sendCommandWithRetry([&]() { pb->set(DeviceCmd::ImuCollect, static_cast<int>(FacSwitch_START)); }); }},
        {"获取按钮状态", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::ButtonState, 1); }); }},

        {"获取IMU校准结果", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::GetImuCaliResult); }); }},

        {"获取连接信息", [&]() { sendCommandWithRetry([&]() { pb->get(DeviceCmd::ConnectInfo); }); }},
