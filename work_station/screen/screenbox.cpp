...
screenbox::screenbox(QWidget *parent) : box_base(parent), ui(new Ui::screenbox)
{
    ...
    ui->statusbar->addPermanentWidget(
        new QLabel(SCREEN_VER + QString(__DATE__) + " " + QString(__TIME__)));
}
...
void screenbox::checkAllTest(int fixtureNumber)
{
    ...
    if (checkStateReady(FixTureStates))
    {
        ...
        reply = QMessageBox::question(this, "屏幕测试", "屏幕正常吗？",
                                      QMessageBox::Yes | QMessageBox::No);
        if (reply == QMessageBox::No)
        {
            bool ok;
            QString text = QInputDialog::getText(nullptr, "屏幕测试", "请输入坏屏号",
