...
PlcModbusSession makeSession(InovanceH5uModbusTcp& tcp, int stationIndex, const LogFn& log, const IsContinueFn& isContinue) {
    return PlcModbusSession(&tcp, PlcStationConfig::fromSettings(stationIndex), log, isContinue);
}
...
bool exec(InovanceH5uModbusTcp& tcp, int stationIndex, LogFn log, IsContinueFn isContinue, PlcCmd cmd,
    ...
    PlcModbusSession session = makeSession(tcp, stationIndex, log, isContinue);

    switch (cmd) {
    case PlcCmd::Connect: {
        if (param.canConvert<QVariantMap>()) {
            const QVariantMap map = param.toMap();
            PlcStationConfig cfg = PlcStationConfig::fromSettings(stationIndex);
            ...
            PlcModbusSession customSession(&tcp, cfg, log, isContinue);
    ...
    case PlcCmd::ReadCoils: {
        ...
        const PlcStationConfig cfg = PlcStationConfig::fromSettings(stationIndex);
        if (!tcp.readMCoils(req.m, req.quantity, cfg.mCoilAddressOffset, cfg.unitId, requestMs, &bits, errorMessage)) {
            return false;
        }
        if (result) {
    ...
    default:
        if (errorMessage) {
            *errorMessage = QStringLiteral("不支持的 PlcCmd: %1").arg(static_cast<int>(cmd));
        }
        return false;
    }
}
