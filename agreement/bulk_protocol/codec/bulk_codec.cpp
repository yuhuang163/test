...
std::atomic<uint16_t> DjiBulkCodec::m_seqNum{0};
...
uint16_t DjiBulkCodec::duss_util_crc16_calc(const uint8_t* data, uint32_t len, uint16_t init_crc) {
...
uint8_t DjiBulkCodec::crc8_calc(const uint8_t* data, uint32_t len, uint8_t init_crc) {
...
QByteArray DjiBulkCodec::buildPacket(uint8_t receiver, uint8_t cmdType, uint8_t cmdSet,
                                     uint8_t cmdID, const QByteArray& data,
                                     uint8_t encryptionType, uint8_t isResponse) {
    QByteArray packet;
    packet.append(char(0x55));
    uint16_t len = 1 + 2 + 1 + 1 + 1 + 2 + 1 + 1 + 1 + data.size() + 2;
    uint16_t version = 1;
    uint16_t vl = ((version & 0x3F) << 10) | (len & 0x03FF);
    packet.append(char(vl & 0xFF));
    ...
    typeByte = ((isResponse & 0x01) << 7) | ((cmdType & 0x03) << 5) | ((reserve & 0x01) << 4) | (encryptionType & 0x0F);
    ...
    packet.append(char(crc16 & 0xFF));
    packet.append(char((crc16 >> 8) & 0xFF));
    return packet;
}
...
void DjiBulkCodec::feed(QByteArray& buffer, const FrameHandler& onFrame) {
    ...
    while (buffer.size() >= minHeaderSize + crcSize) {
        ...
        quint16 verLen = quint8(buffer[1]) | (quint8(buffer[2]) << 8);
        ...
        frame.encryptionType = typeByte & 0x0F;
        frame.receiver = quint8(packet[5]);
...
// md5 implementation placeholder (you would copy md5 implementation here from crc_md5.cpp)
...
void DjiBulkCodec::md5_init(md5_state_t* pms) {
...
void DjiBulkCodec::md5_append(md5_state_t* pms, const md5_byte_t* data, int nbytes) {
...
void DjiBulkCodec::md5_finish(md5_state_t* pms, md5_byte_t digest[16]) {
