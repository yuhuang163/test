...
void Fixture_uart::readFixtureSerialPortData()
{
    ...
    write_len = fixRingBuf->usmile_ring_buffer_write(
        &p_fixRingBuffer, reinterpret_cast<uint8_t *>(dataTemp.data()), dataTemp.size());
...
void Fixture_uart::solve_frame(void)
{
    while (true)
    {
        ...
        if (head->magic == EXT_UART_PHY_LAYER_MAGIC)
        {
            ...
            {
                qDebug() << "head content:"
                         << QByteArray(reinterpret_cast<char *>(head), head->length * 2).toHex();
                qDebug() << "尾巴校验失败" <<frame_buf[head->length-1]<< head->end << head->chargingCurrent << head->magic;
            }
...
void Fixture_uart::save_Fixture_uart_log(int txrx, QByteArray data)
{
    ...
    if (logFile.open(QIODevice::Append | QIODevice::Text))
    {
        ...
        if (txrx)   // 发送的
        {
            out << detailedTimestamp << tr("- 发送的原始数据为：") << data << "\n";
            out << detailedTimestamp << tr("- 发送的16进制数据：") << hexData << "\n";
        }
        else
        {
            out << detailedTimestamp << tr("- 接收的原始数据为：") << data << "\n";
            out << detailedTimestamp << tr("- 接收的16进制数据：") << hexData << "\n";
        }
...
void Fixture_uart::processReceivedData(const QByteArray &data)
{
    ...
    qDebug() << static_cast<int>(receivebuf.at(0))
...
void Fixture_uart::sendFixtureData(FixtureState fixstate)
{
    ...
    case STATE_RELAY4_RESET:
        fixtureSerialPort->write(QByteArray::fromHex("5501040000"));   // 继电器4复位
    }
}

void Fixture_uart::send_start_command(int i)
{
    const char *commands[] = {"AA01", "AA02", "AA03", "AA04", "AA05", "AA06",
                              "AA07", "AA08", "AA09", "AA0A", "A1"};
...
{
    const char *commands[] = {"CC01", "CC02", "CC03", "CC04", "CC05", "CC06",
                              "CC07", "CC08", "CC09", "CC0A", "A1"};
...
{
    const char *commands[] = {"EE01", "EE02", "EE03", "EE04", "EE05", "EE06",
                              "EE07", "EE08", "EE09", "EE0A", "A1"};
