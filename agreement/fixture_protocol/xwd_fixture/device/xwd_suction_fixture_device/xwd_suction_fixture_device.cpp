...
bool XwdSuctionFixtureDevice::sendRawText(QSerialPort* port, const QString& text, QString* errorMessage) {
    if (!port) {
        if (errorMessage)
    ...
    const QByteArray frame = XwdSuctionUartCodec::encodeRawText(text, &parsedAsHex);
    if (frame.isEmpty()) {
        if (errorMessage)
    ...
    if (parsedAsHex)
        ...
        qDebug().noquote() << "XWD_SUCTION FIXTURE TX(原文):" << text.trimmed();
