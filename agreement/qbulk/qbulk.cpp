...
void QBulk::registerCommand() {
    ...
    djifactoryCommandList[djiFactroyCmd_get_active] =
        std::bind(&QBulk::process_dji_get_active, this, std::placeholders::_1);

    djifactoryCommandList[djiFactroyCmd_get_product_status] =
        std::bind(&QBulk::process_dji_get_product_status, this, std::placeholders::_1);

    djifactoryCommandList[djiFactroyCmd_get_product_dbg_misc_subcmd_count] =
        std::bind(&QBulk::process_get_product_dbg_misc_subcmd_count, this, std::placeholders::_1);

    djifactoryCommandList[djiFactroyCmd_sec_dbg_command_req_handler] =
        std::bind(&QBulk::process_sec_dbg_command_req_handler, this, std::placeholders::_1);


    djifactoryCommandList[djiFactroyCmd_sec_dbg_command_auth_handler] =
        std::bind(&QBulk::process_sec_dbg_command_auth_handler, this, std::placeholders::_1);


    djifactoryCommandList[djiFactroyCmd_anti_rollback_comm] =
        std::bind(&QBulk::process_get_anti_rollback_comm, this, std::placeholders::_1);
    ...
    djifactoryCommandList[djiFactroyCmd_sys_event_report_status] =
        std::bind(&QBulk::process_sys_event_report_status, this, std::placeholders::_1);
    djifactoryCommandList[djiFactroyCmd_set_product_status] =
        std::bind(&QBulk::process_set_product_status, this, std::placeholders::_1);
    djifactoryCommandList[djiFactroyCmd_read_root_key_status] =
        std::bind(&QBulk::process_read_root_key_status, this, std::placeholders::_1);
...
QByteArray QBulk::buildPacket( uint8_t receiver,
                             uint8_t cmdType,
                              ...
                              const QByteArray &data, uint8_t encryptionType,uint8_t isResponse)
{
    QByteArray packet;
...
void QBulk::get_device_date() {
    QByteArray v1data;
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0801), 2, 0, 0x4b, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时

}
void QBulk::get_product_status() {
    QByteArray v1data;
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0xc5, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::get_product_dbg_misc_subcmd_count() {
    ...
    v1data.append(char(0x02));
        ...
        v1data.append(char(0x00));
            v1data.append(char(0x00));
            v1data.append(char(0x00));
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0xe0, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::get_Esdd_Check_Antirollback() {
    ...
    v1data.append(char(0x00));
        ...
        QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0803), 2,
                                        0, 0x36, v1data);
           bulkWrite(pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::get_product_active() {
    ...
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0x32, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::set_device_restory_setting() {
    QByteArray v1data;
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x1704), 2, 0x03, 0xf3, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::set_product_dbg_count(){
    ...
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0xe1, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
}


void QBulk:: set_sec_dbg_auth_req_one_func(
    const QString& snStr,
...
#pragma pack(push, 1)
    typedef struct {
        ...
        uint32_t perm;           // 授权的权限
        uint32_t count;          // 授权次数。不过使用次数，对QA测试会有一些问题。0xFFFFFFFF可以不限制
...
#pragma pack(pop)
    ...
    memcpy(req.name,
           name.constData(),
           qMin<int>(name.size(), sizeof(req.name)));
    ...
    // 抓包里的 CMAC（16 字节）
    static const uint8_t cmac_data[16] = {
        0x72, 0x7f, 0xd7, 0xf3,
        0x0b, 0xd4, 0xb3, 0x34,
        0xbd, 0x2d, 0xc4, 0x6a,
        0x5c, 0xc2, 0x92, 0x30
    ...
    static const uint8_t sig_data[64] = {
        0x25, 0x53, 0x70, 0x57, 0x57, 0x00, 0x49, 0x33,
        0x52, 0x3f, 0xe5, 0xfa, 0xd2, 0xf1, 0x39, 0x4d,
        0xa1, 0xd1, 0x96, 0xa6, 0x6c, 0x45, 0x0d, 0xf7,
        0x38, 0x95, 0x37, 0x21, 0xa5, 0x84, 0x7c, 0x80,
        0xed, 0xd2, 0x13, 0xba, 0xd6, 0xe2, 0x1f, 0x87,
        0x92, 0x4c, 0x61, 0xa2, 0x17, 0xf1, 0x11, 0xcf,
        0x49, 0x37, 0x4b, 0x28, 0x5a, 0x05, 0x8e, 0xd6,
        0x45, 0x3a, 0xeb, 0x97, 0x3a, 0xe4, 0x63, 0xe5
    };
    ...
    // payload
    v1data.append(reinterpret_cast<const char*>(&req),
                  sizeof(req));


    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0xe2, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
//         uint8_t cmac[32]; /* only the first 16 bytes are used *///72 7f d7 f3 0b  d4 b3 34 bd  2d c4 6a 5c  c2 92 30 00 00 00 00 00  00 00 00 00  00 00 00 00  00 00 00
//         uint8_t sig[64];  /* esdsa signature *///25  53 70 57 57  00 49 33 52  3f e5 fa d2  f1 39 4d a1 d1 96 a6 6c  45 0d f7 38  95 37 21 a5  84 7c 80 ed d2 13 ba d6  e2 1f 87 92  4c 61 a2 17   f1 11 cf 49  37 4b 28 5a  05 8e d6 45  3a eb 97 3a  e4 63 e5
...
void QBulk::set_device_date() {
    ...
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0801), 2, 0, 0x4a, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::get_root_key_status(){
    ...
    QByteArray pkt = buildPacket(
        HOST_ID_FROM_16BIT_TO_8BIT(0x0804),
        2,
        0,
        0xc6,
        v1data
...
emit send_bulk_data("当前PC端收到的窗口内的最大值"+QString::number(lost_list_rsp_seq));
    ...
    if (written != payload.size()) {
        qWarning() << "[2A] write size mismatch:"
    ...
    if (m_totalFileSize > 0) {
        progress = static_cast<int>(
            (m_receivedSize * 100) / m_totalFileSize);
    }
...
void QBulk::set_write_product_status(){
    ...
    // v1data.append(char(0x00));
    // v1data.append(char(0x00));
    QByteArray pkt = buildPacket(HOST_ID_FROM_16BIT_TO_8BIT(0x0804), 2, 0, 0xc4, v1data);
    bulkWrite( pkt, 1000); // 发到 OUT endpoint 0x01，100ms超时
...
void QBulk::process_dji_2a_send_file(QByteArray &f)
{
 ...
 emit sendGetDjiResponse(1,info.retCode);
    ...
    if(f.size()==5)//窗口的文件第几包
    {
        ...
        emit send_bulk_data("当前设备端收到的窗口内的最大值"+QString::number(window_size));
        two_a_can_send=1;
        return;
    }
...
#pragma pack(pop)
    ...
    const DeviceInfo* info =
        reinterpret_cast<const DeviceInfo*>(f.constData());
...
static QString checkErrToString(uint8_t err)
{
 ...
 emit sendGetDjiResponse(1,info.retCode);
    ...
    if(f.size()==3)
    {
         ...
         emit send_bulk_data(QString("ab分区校验失败值分别为=%1 %2").arg(p[1]).arg(p[2]));
        if(info.retCode==0x00)
             emit send_bulk_data("ab分区校验成功");
        else
            emit send_bulk_data(QString(
                                    "AB 分区校验失败："
                                    "A[%1: %2], B[%3: %4]"
                                    ).arg(p[1])
...
#pragma pack(pop)
    ...
    if (f.size() < sizeof(DeviceInfo)) {
        qWarning() << "sec dbg response too short:"
                   << f.size()
 ...
 emit sendGetDjiResponse(1,info.retCode);
     ...
     QString snStr = QString::fromLatin1(
         reinterpret_cast<const char*>(info.sn),
         snLen
         );

     emit send_bulk_data(
         QStringLiteral("SN=%1").arg(snStr)
         );
     ...
     set_sec_dbg_auth_req_one_func(snStr, "o-joy",  0x70,0x32,0x1c0107ea,info.nonce);
 }
...
void QBulk::process_dji_get_active(QByteArray& f)
{
   ...
   info.retCode = p[0];
    ...
    // 5️⃣ board_num（变长）
    if (offset + board_num_length > len) return;
    QByteArray board_num(
        reinterpret_cast<const char*>(p + offset),
        board_num_length);
...
void QBulk::process_djiFactroyCmd_get_version(QByteArray& f){
 ...
 emit sendGetDjiResponse(1,info.retCode);
    ...
    uint32_t flags =
      ...
      info.is_mass_product =
        flags & (1u << 31);

      info.is_support_safe_update =
        flags & (1u << 30);
