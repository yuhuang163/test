...
void hqmes::LogIn(MesPacketData pack)
{
    if (pack.factory == "hq")
    {
        ...
        requestData["HEAD"] =
            QJsonObject{{"H_GUID", "F60A9855F87E4510A3CA39045C57325E"}, {"H_OP", "MesGetToken"}};
        requestData["MAIN"] = QJsonObject{
            {"G_USER", pack.userNo},
            {"G_PASSWORD", pack.password},   // encryption(password)
            {"G_OP_PC", hostName},
            {"G_HOSTINFO",
             QJsonArray{QJsonObject{{"G_OP_IP", IP_Adress}, {"G_OP_MAC", MAC_Adress}}}}};
        ...
        if (reply->error() == QNetworkReply::NoError)
        {
            ...
            else
            {
                if (jsonObj.contains("H_MSG"))
                {
                    QString resultMsg = jsonObj.value("H_MSG").toString();
                    qDebug() << "LogIn failed. Error: " << resultMsg;
                    emit operateMesError(pack.mechines, "过站失败：" + resultMsg);
                }
                else
                {
                    emit operateMesError(pack.mechines, "过站失败，没有报错信息");
                }
            }
        }
        else
